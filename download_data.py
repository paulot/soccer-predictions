...
def build_self_contained_pipeline(
    max_matches_per_comp: int = 30, ref_date: str = "2026-06-30", decay_lambda: float = 0.0019
) -> None:
    ...
    player_pass_counts: Dict[str, Dict[str, float]] = (
        {}
    )  # player -> {completions, attempts, progressive} (all weighted)
    ...
    manager_stats: Dict[str, Dict[str, float]] = (
        {}
    )  # manager -> {weighted_directness, weighted_width, weighted_tempo, weight_sum}
        ...
        try:
            ...
            for _, match_row in match_slice.iterrows():
                ...
                if not passes.empty:
                    ...
                    passes["end_zone"] = passes["pass_end_location"].apply(
                        lambda loc: map_coordinates_to_zone(loc[0], loc[1])
                    )
                    passes["event_type"] = passes["pass_outcome"].apply(
                        lambda outcome: "Turnover" if pd.notnull(outcome) else "Pass"
                    )
                ...
                if not def_events.empty:
                    ...
                    def_events["zone"] = def_events["location"].apply(
                        lambda loc: map_coordinates_to_zone(loc[0], loc[1])
                    )
    ...
    transitions = (
        successful_passes.groupby(["start_zone", "end_zone"])["weight"].sum().reset_index(name="weighted_count")
    )
    ...
    df_player_profiles = df_player_profiles[
        (df_player_profiles["attempts"] >= 5.0) | (df_player_profiles["shots"] >= 1.0)
    ]
    ...
    for gk, stats in goalkeeper_counts.items():
        ...
        gk_records.append(
            {"goalkeeper": gk, "shots_faced": faced, "goals_conceded": conceded, "save_ratio": save_ratio}
        )
    ...
    for team, zones_dict in team_defensive_actions.items():
        ...
        for zone, count in zones_dict.items():
            ...
            def_records.append({"team": team, "zone": zone, "defensive_rate": rate})
...
def main(max_matches: int, ref_date: str, decay_lambda: float) -> None:
    build_self_contained_pipeline(max_matches_per_comp=max_matches, ref_date=ref_date, decay_lambda=decay_lambda)
