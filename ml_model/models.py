...
class HeuristicTransitionModel(BaseTransitionModel):
    ...
    def get_transition_probabilities(
        ...
        **kwargs: Any,
        ...
        current_team: str = (
            player_to_team.get(player_on_ball, "") if player_on_ball else str(np.random.choice([home_team, away_team]))
        )
    ...
    def get_turnover_probability(
        ...
        **kwargs: Any,
        ...
        if player_on_ball and player_on_ball in player_profiles:
            return float(
                max(0.05, min(0.30, (1.0 - player_profiles[player_on_ball]["accuracy"]) * 0.5 + 0.05 + def_factor))
            )
        else:
...
class MLTransitionModel(BaseTransitionModel):
    ...
    def _compile_features(
        ...
        pass_angle: float = 0.0,
        possession_directions: Optional[List[int]] = None,
    ...
    def get_transition_probabilities(
        ...
        if isinstance(self.destination_model, torch.nn.Module) or hasattr(self.destination_model, "forward"):
            with torch.no_grad():
                continuous_cols = [
                    c for c in df_feats.columns if c not in ["player_role", "start_zone_x", "start_zone_y"]
                ]
