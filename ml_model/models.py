...
class BaseTransitionModel:
    ...
    """
    def get_transition_probabilities(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                                      manager_profiles: Dict[str, Dict[str, float]], team_to_manager: Dict[str, str], player_to_team: Dict[str, str], 
                                      home_team: str, away_team: str, zones: List[str], **kwargs: Any) -> pd.Series:
        raise NotImplementedError
        
    def get_turnover_probability(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                                  team_defensive_profiles: Dict[str, Dict[str, float]], player_to_team: Dict[str, str], home_team: str, away_team: str, **kwargs: Any) -> float:
        raise NotImplementedError
...
class HeuristicTransitionModel(BaseTransitionModel):
    ...
    """
    def __init__(self, base_matrix: pd.DataFrame) -> None:
        self.base_matrix: pd.DataFrame = base_matrix
 
    def get_transition_probabilities(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                                      manager_profiles: Dict[str, Dict[str, float]], team_to_manager: Dict[str, str], player_to_team: Dict[str, str], 
                                      home_team: str, away_team: str, zones: List[str], **kwargs: Any) -> pd.Series:
        # Get baseline transition probabilities
        zone_probs: pd.Series = self.base_matrix.loc[current_zone].copy()
        if zone_probs.sum() == 0:
            return zone_probs
        ...
        current_team: str = player_to_team.get(player_on_ball, "") if player_on_ball else str(np.random.choice([home_team, away_team]))
        mgr_name: str = team_to_manager.get(current_team, "")
        mgr = manager_profiles.get(mgr_name, {"directness": 5.0, "width": 5.0, "tempo": 5.0})
    ...
    def get_turnover_probability(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
        ...
        if player_on_ball and player_on_ball in player_profiles:
            return float(max(0.05, min(0.30, (1.0 - player_profiles[player_on_ball]['accuracy']) * 0.5 + 0.05 + def_factor)))
        else:
...
class MLTransitionModel(BaseTransitionModel):
    ...
    """
    def __init__(self, outcome_model_path: str, destination_model_path: str) -> None:
        if "neural_network" in outcome_model_path:
            self.outcome_model: Any = torch.jit.load(outcome_model_path)
            self.destination_model: Any = torch.jit.load(destination_model_path)
        else:
        ...
        with open("data/embeddings/zone_embeddings.pkl", 'rb') as f:
            self.zone_embeddings: Dict[str, np.ndarray] = pkl.load(f)
        with open("data/embeddings/player_embeddings.pkl", 'rb') as f:
            self.player_embeddings: Dict[str, np.ndarray] = pkl.load(f)
        with open("data/embeddings/manager_embeddings.pkl", 'rb') as f:
            self.manager_embeddings: Dict[str, np.ndarray] = pkl.load(f)
            
        # Load goalkeeper profiles
        try:
            df_gk: pd.DataFrame = pd.read_csv("data/goalkeeper_profiles.csv")
            self.gk_profiles: Dict[str, Dict[str, float]] = df_gk.set_index('goalkeeper').to_dict(orient='index')
        except Exception:
            self.gk_profiles = {}
        ...
        try:
            ...
            self.def_profiles: Dict[Tuple[str, str], float] = {}
            for _, row in df_def.iterrows():
                self.def_profiles[(str(row['team']), str(row['zone']))] = float(row['defensive_rate'])
        except Exception:
            self.def_profiles = {}
        ...
        try:
            with open("data/models/player_roles.json", 'r') as f:
                self.player_roles: Dict[str, int] = json.load(f)
        except Exception:
            self.player_roles = {}
        ...
        try:
            with open("data/models/neural_network_outcome_scaler.pkl", 'rb') as f:
                self.outcome_scaler: Any = pkl.load(f)
            with open("data/models/neural_network_destination_scaler.pkl", 'rb') as f:
                self.destination_scaler: Any = pkl.load(f)
        except Exception:
            self.outcome_scaler = None
            self.destination_scaler = None
            
        # Caches to avoid expensive predict_proba calls in the MCMC loop
        self.cache_probs: Dict[Tuple[Any, ...], pd.Series] = {}
        self.cache_turnover: Dict[Tuple[Any, ...], float] = {}
            
    def _compile_features(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                          manager_profiles: Dict[str, Dict[str, float]], team_to_manager: Dict[str, str], player_to_team: Dict[str, str], 
                          home_team: str, away_team: str, history: Optional[List[Tuple[int, int, int]]] = None, score_differential: int = 0, possession_duration: float = 0.0,
                          pass_sequence_index: int = 0, pass_length: float = 0.0, pass_angle: float = 0.0, possession_directions: Optional[List[int]] = None,
                          time_ratio: float = 0.0, under_pressure: int = 0, next_zone: Optional[str] = None) -> Dict[str, float]:
        current_team: str = player_to_team.get(player_on_ball, "") if player_on_ball else str(np.random.choice([home_team, away_team]))
        defending_team: str = away_team if current_team == home_team else home_team
        mgr_name: str = team_to_manager.get(current_team, "")
        
        # Look up embeddings
        z_emb: np.ndarray = self.zone_embeddings.get(current_zone, np.zeros(4))
        p_emb: np.ndarray = self.player_embeddings.get(player_on_ball or "", np.zeros(8))
        m_emb: np.ndarray = self.manager_embeddings.get(mgr_name, np.zeros(4))
        
        # Look up defensive pressure in current_zone
        opp_def_rate: float = self.def_profiles.get((defending_team, current_zone), 0.0)
        
        # Look up opponent goalkeeper and their save ratio
        opp_gk: Optional[str] = None
        for p, t in player_to_team.items():
            if t == defending_team and p in self.gk_profiles:
                opp_gk = p
                break
        opp_gk_profile = self.gk_profiles.get(opp_gk or "", {'save_ratio': 0.70})
        opp_gk_save: float = opp_gk_profile.get('save_ratio', 0.70)
        ...
        game_state_momentum: float = score_differential * (1.0 + time_ratio)
        
        # Calculate pressure differential if next_zone is provided
        pressure_differential: float = 0.0
        if next_zone:
            end_def_rate: float = self.def_profiles.get((defending_team, next_zone), 0.0)
            pressure_differential = end_def_rate - opp_def_rate
        
        feats: Dict[str, float] = {
            'start_zone_x': float(int(current_zone.split('_')[1])),
            'start_zone_y': float(int(current_zone.split('_')[2])),
            ...
            'pass_sequence_index': float(pass_sequence_index),
            ...
            'prev_1_success': float(p1[2]),
            'prev_2_success': float(p2[2])
        }
        ...
        # Add embeddings
        for d in range(4):
            feats[f'zone_emb_{d}'] = float(z_emb[d])
        for d in range(8):
            feats[f'player_emb_{d}'] = float(p_emb[d])
        for d in range(4):
            feats[f'manager_emb_{d}'] = float(m_emb[d])
        for d in range(4):
            feats[f'prev_1_zone_emb_{d}'] = float(p1_emb[d])
            feats[f'prev_2_zone_emb_{d}'] = float(p2_emb[d])
            
        return feats

    def get_transition_probabilities(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                                      manager_profiles: Dict[str, Dict[str, float]], team_to_manager: Dict[str, str], player_to_team: Dict[str, str], 
                                      home_team: str, away_team: str, zones: List[str], history: Optional[List[Tuple[int, int, int]]] = None, score_differential: int = 0, possession_duration: float = 0.0,
                                      pass_sequence_index: int = 0, possession_directions: Optional[List[int]] = None, time_ratio: float = 0.0, under_pressure: int = 0) -> pd.Series:
        if history is None:
            history = [(-1, -1, -1), (-1, -1, -1)]
        ...
        df_feats: pd.DataFrame = pd.DataFrame([feats])
        ...
        if isinstance(self.destination_model, torch.nn.Module) or hasattr(self.destination_model, 'forward'):
            with torch.no_grad():
        ...
        zone_probs: pd.Series = pd.Series(0.0, index=zones)
        if isinstance(self.destination_model, torch.nn.Module) or hasattr(self.destination_model, 'forward'):
            for idx, prob in enumerate(dest_probs):
                z_x = idx // 5
                z_y = idx % 5
                zone_name = f"Z_{z_x}_{z_y}"
                zone_probs[zone_name] = float(prob)
        else:
            for idx, prob in enumerate(dest_probs):
    ...
    def get_turnover_probability(self, current_zone: str, player_on_ball: Optional[str], player_profiles: Dict[str, Dict[str, float]], 
                                  team_defensive_profiles: Dict[str, Dict[str, float]], player_to_team: Dict[str, str], home_team: str, away_team: str, 
                                  history: Optional[List[Tuple[int, int, int]]] = None, score_differential: int = 0, possession_duration: float = 0.0,
                                  pass_sequence_index: int = 0, next_zone: Optional[str] = None, possession_directions: Optional[List[int]] = None,
                                  time_ratio: float = 0.0, under_pressure: int = 0) -> float:
        ...
        cx: int = int(current_zone.split('_')[1])
        cy: int = int(current_zone.split('_')[2])
        nx: int = int(next_zone.split('_')[1])
        ny: int = int(next_zone.split('_')[2])
        dx: float = float(nx - cx) * 20.0
        dy: float = float(ny - cy) * 16.0
        ...
        turnover_prob: float = 0.12
        if isinstance(self.outcome_model, torch.nn.Module) or hasattr(self.outcome_model, 'forward'):
            with torch.no_grad():
                continuous_cols = [c for c in df_feats.columns if c not in ['player_role', 'start_zone_x', 'start_zone_y']]
                df_feats_scaled = df_feats.copy()
                df_feats_scaled[continuous_cols] = self.outcome_scaler.transform(df_feats[continuous_cols])
                feats_tensor = torch.FloatTensor(df_feats_scaled.values)
                turnover_prob = float(self.outcome_model(feats_tensor).item())
        else:
            outcome_probs = self.outcome_model.predict_proba(df_feats)[0]
            turnover_prob = float(outcome_probs[1]) if len(outcome_probs) > 1 else 0.12
