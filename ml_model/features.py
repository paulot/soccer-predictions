...
import pickle
import json
from utils import parse_location, TEAM_TO_MANAGER
...
def extract_features_and_targets(mode: str = "iteration") -> None:
    ...
    for idx, filename in enumerate(match_files):
        ...
        player_to_team: Dict[str, str] = (
            df_events.dropna(subset=["player", "team"]).set_index("player")["team"].to_dict()
        )
        ...
        for _, row in df_events.iterrows():
            ...
            if etype == "Pass":
                ...
                under_pressure: int = 1 if row.get("under_pressure") is True or row.get("under_pressure") == 1.0 else 0
