...
def run_ml_backtest(model_type: str = 'random_forest', mode: str = 'iteration', num_simulations: int = 500) -> Optional[Dict[str, float]]:
    print(f"\n==================================================")
    ...
    for target_mid in list(all_events.keys()):
        ...
        player_to_team: Dict[str, str] = target_events.dropna(subset=['player', 'team']).set_index('player')['team'].to_dict()
        ...
        sim_args = (
            model_type, outcome_path_sim, dest_path_sim, base_matrix,
            home_team, away_team, df_target_passes, player_profiles, gk_profiles, 
        ...
        predicted_outcome: str = 'W' if prob_win > prob_draw and prob_win > prob_loss else ('D' if prob_draw > prob_loss else 'L')
