...
def train_all_embeddings(mode: str = "iteration") -> None:
        ...
        for _, row in passes.iterrows():
            ...
            if pd.notnull(player):
                if player not in player_stats:
                    player_stats[player] = {"start": np.zeros(30), "end": np.zeros(30), "successes": 0, "total": 0}
            ...
            if mgr_name:
                if mgr_name not in manager_stats:
                    manager_stats[mgr_name] = {"start": np.zeros(30), "end": np.zeros(30)}
    ...
    zone_prob_matrix: np.ndarray = np.divide(
        zone_transition, row_sums, out=np.zeros_like(zone_transition), where=row_sums != 0
    )
    ...
    manager_embeddings: Dict[str, np.ndarray] = {
        manager_names[i]: manager_emb_matrix[i] for i in range(len(manager_names))
    }
